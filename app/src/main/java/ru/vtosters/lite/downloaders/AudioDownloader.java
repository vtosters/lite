...
public class AudioDownloader {
    ...
    public static Future<?> downloadPlaylist(Playlist playlist) {
    ...
    public static Future<?> downloadAudio(MusicTrack track) {
        return VTExecutors.getMusicDownloadExecutor().submit(() -> downloadM3U8(track, false));
    }

    public static Future<?> cacheTrack(MusicTrack track) {
        return VTExecutors.getMusicDownloadExecutor().submit(() -> {
    ...
    public static Future<?> cachePlaylist(Playlist playlist) {
        return VTExecutors.getMusicDownloadExecutor().submit(() -> {
            List<MusicTrack> tracks = PlaylistConverter.getPlaylist(playlist);
    ...
    public static Future<?> cacheAllAudios() {
        return VTExecutors.getMusicDownloadExecutor().submit(() -> {
    ...
    public static Future<?> downloadAllAudios() {
        return VTExecutors.getMusicDownloadExecutor().submit(() -> {
            List<MusicTrack> tracks = AudioGet.getAudios();
